...
export default {
  ...
  methods: {
    async submitRegister() {
      ...
      if (!this.fullName) {
        alert('Требуется ввести пароль'); //В тестах ошибка, здесь должно выводиться "Требуется ввести полное имя"
        return;
      }
      if (!this.password) {
      ...
      const { statusCode, message, id, fullName, email } = await register(
        this.email,
        this.fullName,
        this.password,
      );
